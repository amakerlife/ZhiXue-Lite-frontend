...
const VerifyEmailPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Status Card */}
      {status === "loading" && <StatusAlert variant="info" message={message} />}
      {status === "success" && (
        <StatusAlert variant="success" message={message} />
      )}
      {status === "error" && <StatusAlert variant="error" message={message} />}
      ...
      {status === "error" && (
        <Card>
          ...
          <CardContent className="space-y-2">
            <Button onClick={handleGoProfile} className="w-full">
              前往个人中心重新发送
            </Button>
