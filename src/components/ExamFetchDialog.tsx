...
export const ExamFetchDialog: React.FC<ExamFetchDialogProps> = ({
  ...
  const canFetchSchoolData = user && (
    examAPI.hasPermission(user, 0, 2) || // FETCH_DATA权限，SCHOOL级别
    examAPI.hasPermission(user, 0, 3)    // FETCH_DATA权限，GLOBAL级别
  );

  const hasGlobalPermission = Boolean(user && examAPI.hasPermission(user, 0, 3));
  const hasSchoolOnlyPermission = Boolean(user && examAPI.hasPermission(user, 0, 2) && !examAPI.hasPermission(user, 0, 3));
  ...
  const handleFetch = () => {
    if (fetchMode === 'self') {
      ...
      if (params.queryType === 'academicYear') {
        ...
        if (params.schoolInYearCode) queryParams.schoolInYearCode = params.schoolInYearCode;
        if (params.examTypeCode) queryParams.examTypeCode = params.examTypeCode;
      }
                  ...
                  <div className="space-y-4">
                    {loading ? (
                      ...
                      <>
                        ...
                        <div className="space-y-2">
                          <Label>查询类型</Label>
                          <Select value={normalizeCode(params.queryType)} onValueChange={(value) => handleParamChange('queryType', value)}>
                            <SelectTrigger>
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              {(selections?.queryTypeList || []).map(item => (
                                <SelectItem key={item.code || '__all__'} value={normalizeCode(item.code)}>
                                  {item.name}
                                </SelectItem>
                              ))}
                            </SelectContent>
                    ...
                    {params.queryType === 'academicYear' && (
                      <>
                        <div className="space-y-2">
                          <Label>学年</Label>
                          <Select value={normalizeCode(params.academicYear || '')} onValueChange={(value) => handleParamChange('academicYear', value)}>
                            <SelectTrigger>
                        ...
                        {params.academicYear && getTermOptions().length > 0 && (
                          <div className="space-y-2">
                            <Label>学期</Label>
                            <Select value={params.termId || ''} onValueChange={handleTermChange}>
                              <SelectTrigger>
                    ...
                    {params.queryType === 'schoolInYear' && (
                      <div className="space-y-2">
                        <Label>级别</Label>
                        <Select value={normalizeCode(params.schoolInYearCode || '')} onValueChange={(value) => handleParamChange('schoolInYearCode', value)}>
                          <SelectTrigger>
                    ...
                    <div className="space-y-2">
                      <Label>报告类型</Label>
                      <Select value={normalizeCode(params.examTypeCode || '')} onValueChange={(value) => handleParamChange('examTypeCode', value)}>
                        <SelectTrigger>
                          <SelectValue placeholder="选择报告类型（可选）" />
                        </SelectTrigger>
                        <SelectContent>
                          {(selections?.examTypeList || []).map(type => (
                            <SelectItem key={type.code || '__all__'} value={normalizeCode(type.code)}>
                              {type.name}
                            </SelectItem>
                          ))}
                        </SelectContent>
