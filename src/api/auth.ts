...
export const authAPI = {
  ...
  getCurrentUser: () => api.get<ApiResponse & { user: User }>("/user/me"),

  bindZhixue: (data: ConnectZhixueRequest) =>
    api.post<ApiResponse>("/user/zhixue/bind", data),

  unbindZhixue: () => api.post<ApiResponse>("/user/zhixue/unbind"),

  getZhixueBindingInfo: () =>
    api.get<ApiResponse & { binding_info: { username: string }[] }>(
      "/user/zhixue/binding_info",
    ),
  ...
  getEmailVerificationStatus: () =>
    api.get<ApiResponse & EmailVerificationStatusResponse>(
